 ...
 */
//->通过元素的样式类名来获取一组元素
//=>strClass：你想通过哪个样式类名来获取，就传递谁
//1、"w1"获取所有拥有w1样式类名的元素
//2、"w3 w2" 获取所有即拥有w3 也拥有w2样式类名的元素
//=>context：[objecct] 上下文，限定在哪一个容器中获取，如果不传递默认值document
function byClass(strClass, context) {
    ...
    var tagList = context.getElementsByTagName("*");//->通过通配符*获取当前上下文中的所有元素标签
    for (var i = 0; i < tagList.length; i++) {
        ...
        //=>使用正则，假设传递进来的样式类名是"w1"，想验证样式类名中有一个是w1，我们把正则写成 var reg=/\bw1\b/;  reg.test("w1 w2")=>true  reg.test("w100 w2") =>false   reg.test("w1-1 w2") =>true发现了一个问题，我们的样式类名中可以包含-，但是正则的\b元字符代表边界，它把-的左右两边也作为边界了，最后一个例子，我们字符串中没有w1这个样式类，但是结果却是true，所以正则不能这样写
