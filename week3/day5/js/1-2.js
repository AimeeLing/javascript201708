...
//->需求升级：我们不仅要获取元素子节点，而且还想能够通过指定标签名来获取到指定标签的元素子节点
...
. //->如果指定了标签名，在所有元素子节点(ary)中进行二次筛选
 ...
 //->如果传递的标签名和当前循环这一项的标签名不相同，我们在数组中把它删掉即可
 var curAry = ary[k],
 curTagName = curAry.tagName.toUpperCase();//->nodeName<=>tagName xxx.tagName：获取当前元素的标签名，一般都是大写，有时候也是小写
 if (curTagName !== tag.toUpperCase()) {
 //->不是我们想要的，在数组中删除它（防止数组塌陷）
 ary.splice(k,1);
 k--;
 }
 }
