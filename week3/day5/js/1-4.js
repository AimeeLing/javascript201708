...
function byClass(strClass, context) {
    ...
    strClass = strClass.replace(/^ +| +$/g, "");//->以多个空格开头，以多个空格结尾（/^\s+|\s+$/g, ""）

    for (var i = 0; i < tagList.length; i++) {
        var curTag = tagList[i],
            curTagClass = curTag.className;
        var reg = new RegExp("(^| +)" + strClass + "( +|$)");//->验证某一个字符串中是否包含我们传递进来的这个样式类名
        reg.test(curTagClass) ? ary.push(curTag) : null;//->如果当前标签的样式类名中包含传递进来的样式类，那么这个标签就是我们想要的，我们把其添加到数组的末尾
    }
    return ary;
}
