...
function hasClass(curEle, strClass) {
    var curEleClass = curEle.className, //->获取当前元素本身的样式类名
        isMatch = true;//->假设是包含的
...
function addClass(curEle, strClass) {
    ...
    for (var i = 0; i < strClass.length; i++) {
        ...
        if (hasClass(curEle, curClass)) continue;//->已经存在就没有必要继续增加了，不存在我们再新增
...
function removeClass(curEle, strClass) {
    ...
    var originalClass = curEle.className.replace(/\s+/g, "  ");//->获取原有的class值：把多个空格都替换为两个空格（防止正则捕获共享一个字符的问题），把字符串的首尾空格也去掉
