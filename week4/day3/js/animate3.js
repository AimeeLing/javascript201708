...
//1、如果用户传递的时候需要按照顺序依次传递，如果顺序混乱，接收的值也乱了
//2、轻易不能出现不传参给默认的情况，因为其中一个不传，那后面传递的值会整体都向前靠一位
//=>真正项目中如果一个方法需要传递的实参很多的话，我们都会告别形参的时代，开启对象统一处理参数的时代
// function animate(curEle, target, duration, effect) {//->effect：可以传递指定的动画效果，不传递默认就是匀速
...
//->d这五个属性我先给它一些默认值，传递的时候可以有些属性值不传递，我们把传递进来的属性值替换默认的值
//1、我们在传递d时候不一定要按照顺序l，只要指定好对应的属性名即可
...
function animate(options) {
    ...
    //->以后操作都使用_default.xxx（麻烦），我们最好再把其每一项都设置为私有的变量
    ...
    /*duration = duration || 1000;
     effect = effect || animationEffect.Linear;*/
    var time = 0,
        begin = {},
        change = {};
    ...
    //=>1、设置定时器实现动画：在实现新动画之前，先把之前没有结束的动画都结束
    ...
    curEle.animateTimer = window.setInterval(function () {
        ...
        for (var key in target) {
            ...
            /*var curL = animationEffect.Linear(time, begin[key], change[key], duration);
             utils.css(curEle, key, curL);*/
        }
