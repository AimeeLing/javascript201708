...
/*function fn(num, callBack) {
 //->callBack:undefined TypeError: callBack is not a function
...
/*function fn(num, callBack) {
 ...
 //1、可以给回调函数传递参数值
 //2、还可以改变回调函数中的this
 callBack && callBack.call(fn, 100, 200);//->arguments.callee:fn
 }
 fn(100, function (a, b) {
 //->a:100 b:200 this:fn
...
//->执行sort方法的时候，把匿名函数作为回调函数传递给了sort，在sort执行的过程中，把匿名函数也执行了（被执行了n次），并且每一次执行都把数组中的当前项和下一项作为参数传递给这个匿名的回调函数
...
function fn(num, callBack) {
    //->在回调函数的宿主函数（回调函数在哪儿执行的，它的宿主函数就是谁，此时回调函数的宿主函数是fn）中，我们可以把回调函数执行，而且在宿主函数中可以接收回调函数的返回结果，以此来进行后续的一些操作
